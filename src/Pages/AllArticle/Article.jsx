...
const Article = ({ aNews }) => {
    ...
    return (
        <div className={`${aNews.isPremium ? 'bg-gradient-to-r from-gray-300 via-purple-300 to-pink-300 h-[560px] p-6 rounded-md relative shadow-md' : 'p-6 h-[560px] rounded-md bg-gray-400 relative shadow-md'}`}>
            {
            ...
            <p><span className='font-bold'>Publisher Name : </span>{aNews.publisher}</p>
            {
                ...
                <>
                <Link to={`/newsDetails/${aNews._id}`}><button className='btn btn-primary absolute bottom-2'>Details</button></Link></> 
                :
                <Link to={`/newsDetails/${aNews._id}`}><button disabled={aNews.isPremium} className='btn btn-primary absolute bottom-2'>Details</button></Link>
